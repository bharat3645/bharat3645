#!/usr/bin/env python3
"""
Self-hosted profile dashboard generator — NEUBRUTALIST edition.
...
shadows (no blur, no gradients), saturated accent blocks, chunky uppercase
mono type.
...
# ---------------------------------------------------------------------------
# Neubrutalist palettes. `ink` is the single high-contrast colour used for
# every border, hard shadow and primary text. `on_accent` is always near-black
# because accent fills are light, saturated colours in both themes.
# ---------------------------------------------------------------------------
DARK = dict(
    page="#0d1117", card="#191921", ink="#f4efe1", muted="#a7a291", faint="#6f6c60",
    on_accent="#141109", grid="#2b2b33",
...
DOMAIN_ACCENT = {
    "Agent Security": "red",
    "AI Infrastructure": "blue",
    "Post-Quantum Crypto": "purple",
    "Backend Systems": "green",
    "Supply Chain": "yellow",
}
...
def gather():
    ...
    for f in facts.FLAGSHIPS:
        name = f["name"]
        if f.get("private"):
            # A cross-repo default GITHUB_TOKEN cannot read a private repo, so
            # counting it would make the CI tally differ between local (PAT) and
            # CI runs. Public-only keeps the number reproducible by any viewer.
            continue
        ...
        if runs and runs.get("workflow_runs"):
            concl = runs["workflow_runs"][0].get("conclusion")
        data["ci"][name] = concl
    if lang_bytes:
...
def line(x1, y1, x2, y2, stroke="ink", sw=2, p=None, dash=None, cap="butt"):
...
def circle(cx, cy, r, fill="ink", stroke=None, sw=2, p=None):
...
def card(x, y, w, h, p, fill="card", accent=None, dx=6, dy=6, rx=3, bw=2.5):
    """A neubrutalist block: hard offset shadow + thick-bordered panel."""
    sh = col(p, "ink")
    out = rect(x + dx, y + dy, w, h, fill=sh, rx=rx, p=p)  # shadow uses ink colour
    out += rect(x, y, w, h, fill=fill, rx=rx, stroke="ink", sw=bw, p=p)
    if accent:  # accent spine on the left edge
...
def tab(x, y, label, accent, p, size=15, h=30, dx=5, dy=5, font=MONO):
    """A brutalist sticker label: accent fill, ink border, hard shadow."""
...
def frame(w, h, p):
    return (f'<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}" '
            ...
            + rect(0, 0, w, h, fill="page", rx=0, p=p))


def head(p, w, title, sub, accent, y=20):
    t, tw = tab(24, y, title, accent, p)
    t += text(24, y + 52, sub, size=12, fill="muted", weight=600, p=p)
    return t
...
def hero(p, d):
    ...
    if withruns == 0:  # API unreachable — fall back to last-verified public tally
    ...
    s.append(text(W - 32, 37, f"PORTFOLIO // github.com/{OWNER}", size=13, fill="on_accent",
                  weight=800, font=MONO, anchor="end", spacing=0.5, p=p))
    # layered name (hard text shadow in accent)
    nx, ny = 36, 100
    s.append(text(nx + 3, ny + 3, facts.NAME.upper(), size=31, fill="pink", weight=800,
    ...
    s.append(f'<rect x="{curx:.1f}" y="{ny+18:.1f}" width="10" height="15" fill="{p["pink"]}">'
    ...
    for i, (val, lab, c) in enumerate(tiles):
        ...
        s.append(text(tx + tw / 2 + 3, y0 + 39, lab, size=8.5, fill="muted", weight=700,
                      font=MONO, anchor="middle", spacing=0.4, p=p))
    s.append("</svg>")
    return "".join(s)


# ---------------------------------------------------------------------------
# Instrument 2 — domain radar + repo index
# ---------------------------------------------------------------------------
SHORT_DOMAIN = {
    "Agent Security": "AGENT SEC",
...
def radar(p, d):
    ...
    N = len(doms)
    s = [frame(W, H, p)]
    s.append(head(p, W, "DOMAIN MAP", "13 flagship repositories · five problem domains · axis = live repo count",
                  "blue"))
    ...
    for ring in (0.34, 0.67, 1.0):
        ...
        s.append(f'<polygon points="{pts}" fill="none" stroke="{p["grid"]}" '
                 f'stroke-width="2"/>')
    for i, dm in enumerate(doms):
        ...
        s.append(text(lx, ly + dy + 13, f"{counts[dm]} REPOS", size=9.5, fill=c, weight=800,
                      anchor=anchor, font=MONO, p=p))
    ...
    for dm in doms:
        ...
        reps = [f for f in facts.FLAGSHIPS if f["domain"] == dm]
        for f in reps:
            iy += 18
...
# ---------------------------------------------------------------------------
# Instrument 3 — benchmark board
# ---------------------------------------------------------------------------
def benchmarks(p, d):
    W, H = 850, 336
    s = [frame(W, H, p)]
    s.append(head(p, W, "VERIFIED BENCHMARKS",
    ...
    for i, b in enumerate(facts.BENCHMARKS):
        ...
        s.append(rect(x + cw - mw - 14, y + 13, mw, 19, fill=c, rx=2, stroke="ink",
                      sw=1.5, p=p))
        ...
        if b.get("bar") is not None:
            ...
            s.append(line(bar_x + fillw, bar_y, bar_x + fillw, bar_y + 9, stroke="ink",
                          sw=2, p=p))
        s.append(text(x + 22, y + 98, b["detail"], size=10, fill="muted", weight=600, p=p))
    s.append("</svg>")
    return "".join(s)


# ---------------------------------------------------------------------------
# Instrument 4 — post-quantum migration clock
# ---------------------------------------------------------------------------
def pqc_clock(p, d):
    W, H = 850, 300
    s = [frame(W, H, p)]
    s.append(head(p, W, "POST-QUANTUM MIGRATION CLOCK",
    ...
    s.append(text(tx0, ty + 20, "TODAY " + today.strftime("%b %Y"), size=9.5,
                  fill="muted", weight=700, font=MONO, p=p))
    for dl in dls:
        fx = tx0 + (dl["days"] / horizon) * (tx1 - tx0)
        c = urgency(dl["days"])
        s.append(line(fx, ty - 11, fx, ty + 11, stroke="ink", sw=3, p=p))
        s.append(rect(fx - 6, ty - 6, 12, 12, fill=c, rx=1, stroke="ink", sw=2, p=p))
    # cards
    ...
    for i, dl in enumerate(dls):
        ...
        s.append(rect(x, cy0, cw, 8, fill=c, rx=0, p=p))
        s.append(line(x, cy0 + 8, x + cw, cy0 + 8, stroke="ink", sw=2.5, p=p))
        s.append(text(x + 14, cy0 + 32, dl["dt"].strftime("%Y-%m-%d"), size=11,
                      fill="muted", weight=700, font=MONO, p=p))
        s.append(text(x + 14, cy0 + 66, f"{dl['days']:,}", size=30, fill="ink",
                      weight=800, font=MONO, p=p))
        s.append(text(x + 14, cy0 + 82, "DAYS LEFT", size=10, fill=c, weight=800,
...
def langmix(p, d):
    ...
    s = [frame(W, H, p)]
    s.append(head(p, W, "LANGUAGE MIX",
    ...
    for lang, v in items:
        ...
        s.append(f'<rect x="{x:.1f}" y="{by}" width="{max(0,seg):.1f}" height="{bh}" '
                 f'fill="{c}"/>')
        if x > bx:
...
# ---------------------------------------------------------------------------
INSTRUMENTS = {
    "hero": hero, "domains": radar, "benchmarks": benchmarks,
